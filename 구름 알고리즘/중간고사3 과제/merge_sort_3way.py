def merge_sort_3way(A, first, last):
    ...
    while i <= mid1 and j <= mid2 and k <= last:
        ...
        if A[i] <= A[j]:
            Mc += 1 # A[i] vs A[k]
            if A[i] <= A[k]:
                B.append(A[i]); Ms += 1; i += 1
            else:
                B.append(A[k]); Ms += 1; k += 1
        else: # A[j] < A[i]
            Mc += 1 # A[j] vs A[k]
            if A[j] <= A[k]:
                B.append(A[j]); Ms += 1; j += 1
            else:
                B.append(A[k]); Ms += 1; k += 1
    ...
    # 남은 요소를 B에 마저 복사합니다.
    for i_ in range(i, mid1 + 1): B.append(A[i_]); Ms += 1
    for j_ in range(j, mid2 + 1): B.append(A[j_]); Ms += 1
    for k_ in range(k, last + 1): B.append(A[k_]); Ms += 1
